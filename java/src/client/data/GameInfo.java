...
public class GameInfo {
	...
	public GameInfo(Game game) {
		...
		for (GetPlayer player : playerList) {
			if (player.getName() != null) {
				players.add(new PlayerInfo(player.getID(), i++, player.getName(), player.getColor()));
			}
		}
	}
	...
	public void update(TransferModel newLump) {
		...
		for (int i = 0; i < players.size(); i++) {
			...
			if (p != null) {
				addPlayer(new PlayerInfo(p.getID(), i, p.getName(), p.getColor()));
			}
		}
	}
	...
	public PlayerInfo getPlayerInfoByColor(CatanColor color) {
		for (PlayerInfo player : players) {
	...
	public PlayerInfo getPlayerInfoByIndex(int playerIndex) {
